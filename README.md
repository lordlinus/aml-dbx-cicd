...
```
dbx execute --cluster-name=<name of interactive cluster> --job=aml-dbx-cicd-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=aml-dbx-cicd-sample-integration-test --files-only
dbx launch --job=aml-dbx-cicd-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `aml_dbx_cicd/__init__.py` file and tag the current code version:
```
